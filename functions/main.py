...
def parakeetAPI(req: https_fn.Request) -> https_fn.Response:
  ...
  if not all([topic, keywords, native_language, target_language]):
...
- sentence_translation: (sentence in {target_language})
- keywords_native: (an array of keywords of the sentence in {native_language})
- keywords_translations: (an array of keywords in {target_language} with matching index of {native_language} keywords list)

For the specifications in parenthesis you must generate the content. The keys shall remain exactly the same. If no keywords are provided add your own keywords to be used in the conversation.
'''
}
      ],
