...
export class AppStorePurchaseHandler extends PurchaseHandler {
  ...
  handleServerEvent = functions.https.onRequest(async (req, res) => {
    ...
    const decodedBody: appleReceiptVerify.SignedPayload = jwtDecode(
      req.body.signedPayload
    );
    const signedInfo: appleReceiptVerify.SignedTransactionInfo = jwtDecode(
      decodedBody.data.signedTransactionInfo
    );
    const eventData = {
    ...
    if (!productData) {
      console.log(
        'No matching product data for product: ' + eventData.productId
      );
      res.status(403).send();
      return;
    }
    ...
    if (productData.type == 'SUBSCRIPTION') {
      try {
        await this.firebaseCalls.updatePurchase({
          ...
          status:
            Date.now() >= parseInt(eventData.expiresDate, 10)
              ? 'EXPIRED'
              : 'ACTIVE',
        });
      } catch (e) {
