...
def top_users_lessons_count(
    lessons_df, users_df, top_n=10, period_days=None, save=True
):
    """
    ...
    user_counts_with_names["name"] = user_counts_with_names["name"].fillna(
        "Unknown User"
    )

    # Sort by lesson count and take top N
    top_users = user_counts_with_names.sort_values(
        "lesson_count", ascending=False
    ).head(top_n)
...
def pie_chart_replay_status(analytics_df, period_days=None, save=True):
    ...
    if save:
        ...
        replay_data = pd.DataFrame({"replay_pattern": labels, "count": sizes})
...
def pie_chart_lessons_distribution(
    lessons_df, users_df, period_days=None, user_creation_days=None, save=True
):
    """
    ...
    if user_creation_days is not None:
        ...
        if "created_at" in users_df.columns:
            ...
            print(
                "Warning: No creation date column found in users_df. Using all users."
            )
    ...
    all_users = pd.merge(
        all_users, user_lesson_counts, how="left", left_on="id", right_on="user_ID"
    )
    ...
    plt.pie(
        ...
        startangle=90,
        shadow=False,
        explode=[0.05] * len(non_zero_sizes),
    )
    period_text = f"Past {period_days} days" if period_days else "All time"
    user_period_text = (
        f", users created in past {user_creation_days} days"
        if user_creation_days
        else ""
    )
    plt.title(
        f"Distribution of Users by Number of Lessons Created ({period_text}{user_period_text})"
    )
    plt.axis("equal")

    if save:
        plt.savefig("img/user_lesson_distribution.png")
        dist_data = pd.DataFrame({"lesson_count": labels, "number_of_users": sizes})
...
def analyze_user_engagement_patterns(lessons_df, users_df, period_days=None, save=True):
    ...
    engagement_data["avg_lessons_per_active_day"] = (
        engagement_data["total_lessons"] / engagement_data["active_days"]
    )
    ...
    for segment in ["One-time User", "Casual User", "Regular User", "Power User"]:
        ...
        percentage = (
            100 * count / segment_counts.sum() if segment_counts.sum() > 0 else 0
        )
    ...
    scatter = sns.scatterplot(
        ...
        y="total_lessons",
        hue="user_segment",
        size="avg_lessons_per_active_day",
        sizes=(20, 200),
        alpha=0.7,
    )
...
def analyze_language_preferences(lessons_df, users_df, period_days=None, save=True):
    ...
    lang_cross_tab = pd.crosstab(
        filtered_df["native_language"], filtered_df["target_language"]
    )
...
def analyze_user_retention(lessons_df, users_df, period_days=None, save=True):
    ...
    filtered_df["date"] = filtered_df["timestamp"].dt.to_period(
        "W"
    )  # 'W' represents weekly periods
    ...
    user_cohorts["period_number"] = (
        user_cohorts["date"] - user_cohorts["cohort"]
    ).apply(lambda x: x.n)
    ...
    retention_table = (
        user_cohorts.drop_duplicates(["user_ID", "cohort", "period_number"])
    ...
    user_retention_details = user_retention_details[
        ["user_ID", "cohort", "period_number"]
    ]

    # Add user information
    user_details = pd.merge(
        user_retention_details,
        users_df[["id", "name", "premium", "language_level", "target_language"]],
        left_on="user_ID",
        right_on="id",
        how="left",
...
def generate_all_charts(period_days=None):
    ...
    pie_chart_lessons_distribution(
        lessons_df, users_df, period_days, user_creation_days=period_days
    )
