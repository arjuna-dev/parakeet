...
class _LevelDetailScreenState extends State<LevelDetailScreen> {
  ...
  Future<int> _getLessonLevel(String documentId) async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        return data?['categoryLevel'] ??
            1; // Default to level 1 if no level specified
      }
    } catch (e) {
      ...
      if (userId != null) {
        ...
        if (mounted) {
          setState(() {
            ...
            _nextCreditReset =
                null; // No longer using credit reset dates in daily system
          });
        }
      }
    } catch (e) {
  ...
  bool _isLevelCompleted() {
    final requiredLessons =
        CategoryLevelService.levelRequirements[widget.levelNumber] ?? 3;
  ...
  Future<void> _handleCreateNewLesson() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(
            'https://europe-west1-noble-descent-420612.cloudfunctions.net/generate_lesson_topic'),
      ...
      // We'll reload anyway just in case.
      _loadLevelLessons();
    } catch (e) {
      print(e);
                          ...
                          Row(
                            children: [
                              Container(
                                ...
                                child: Icon(
                                  CategoryLevelService.getLevelIcon(
                                      widget.levelNumber),
                                  color: levelColor,
                                  size: 32,
                                ),
                            ...
                            children: [
                              ...
                              ClipRRect(
                                ...
                                child: LinearProgressIndicator(
                                  value: (_getCompletedLessonsCount() /
                                          requiredLessons)
                                      .clamp(0.0, 1.0),
                                  minHeight: 12,
                                        ...
                                        child: Text(
                                          _isLevelCompleted()
                                              ? (widget.levelNumber < 3
                                                  ? 'Move to Level ${widget.levelNumber + 1} to continue your journey.'
                                                  : 'You\'ve mastered all levels in this category!')
                                              : () {
                        ...
                        : _generationsRemaining <= 0
                            ? _isPremium
                                ...
                                : () async {
                                    ...
                                    await LessonService.showPremiumDialog(
                                        context);
                                    _loadGenerationsRemaining(); // Refresh credits after dialog
                                  }
                            : _handleCreateNewLesson,
                    backgroundColor: _generationsRemaining <= 0
                            ...
                            : Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    _generationsRemaining <= 0
                                        ? 'Out of credits'
                                        : '$_generationsRemaining credits remaining',
                                    style: TextStyle(
                                      fontWeight: FontWeight.w600,
                                      fontSize: 12,
                                      color: Colors.white.withOpacity(0.9),
